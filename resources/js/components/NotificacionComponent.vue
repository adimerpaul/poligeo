                                                        ...
                                                        <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" @submit.prevent="guardar">
                                                            <div class="item form-group">
                                                                ...
                                                                <div class="col-md-6 col-sm-6 ">
                                                                    <input v-model="noti.titulo" type="text" id="titulo" required="required" placeholder="Nombre" name="titulo" class="form-control ">
                                                                </div>
                                                            </div>
                                                            <div class="item form-group">
                                                                <label  class="col-form-label col-md-3 col-sm-3 label-align" for="contenido">Contenido<span class="required">*</span>
                                                                </label>
                                                                <div class="col-md-6 col-sm-6 ">
                                                                    <input v-model="noti.contenido" type="text" id="contenido" required="required" placeholder="Contenido" name="contenido" class="form-control ">
                                                                </div>
                                                            </div>
                                                            <div class="item form-group">
...
<!--                                                                    <input v-model="noti.tipo" type="text" id="tipo" required="required" placeholder="Nombre" name="name" class="form-control ">-->
                                                                    <select name="tipo" class="form-control" id="" v-model="noti.tipo_id" id="tipo" required>
                                                                        <option value="" selected>Selecionar...</option>
                                                                        <option style="color: white"  v-for="(item,index) in tipo" :key="index"  v-bind:style="{background: item.color}" v-bind:value="item.id">
                                                                            {{item.nombre}}
                                                                        </option>
                                                                    </select>
                                                                </div>
                                        ...
                                        <table id="datatable2" class="table table-striped table-bordered" style="width:100%">
                                            ...
                                            <tr v-for="(item,index) in notificaciones" :key="index">
                                                <td>{{item.id}}</td>
                                                <td>{{item.titulo}}</td>
                                                <td><div v-bind:style="{background:item.color}" style="border-radius: 0.25em;color: white;text-align: center" >{{item.nombre}}</div></td>
                                                ...
                                                <td>
                                                    ...
                                                    <button class="btn btn-sm btn-dark"@click="cambio(item.id)"> <i class="fa fa-eye"></i> </button>
    ...
    export default {
        ...
        methods:{
            ...
            guardar(){
                let cm = this;
                if(this.nuevo){
                    axios.post('/notificaciones',this.noti).then(res=>{
                        // console.log(res);
                    ...
                    axios.put('/notificaciones/'+this.noti.id,this.noti).then(res=>{
                        // console.log(res);
                        // this.tiponotificaiones=res.data;
                        $('.bs-example-modal-lg').modal('hide');
                        this.datos();
            ...
            eliminar(id){
                ...
                this.$fire({
                    ...
                    // console.log(r.value);
                    if(r.value){
                        axios.delete('/notificaciones/'+id).then(res=>{
                            // console.log(res);
                            this.datos();
